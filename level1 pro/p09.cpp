...
//have the possibility of failing to depict the map
...
inline int _dehashx(int num){
    if(num%size)return num/size+1;
    else return num/size;
}
inline int _dehashy(int num){
    if(num%size==0)return size;
    else return num-num/size*size;
}

int father(int x,int y){
    if(belong[x][y]==_hash(x,y))return belong[x][y];
...
void print_map()
{
    ...
    for(int i=0;i<=size+1;++i){
        for(int j=0;j<=size+1;++j){
            if(_map[i][j]==-1)putchar('x');
            else if(_map[i][j])putchar('0');
            else putchar(' ');
            putchar(' ');
        }
        printf("\n");
...
inline bool valid(int x,int y){
    if(_map[x][y]==0)return false;
    if((x<=3&&y<=3)||(x>=size-2&&y>=size-2))return true;
    int tot=0;
    if(_map[x-1][y])tot++;
    if(_map[x+1][y])tot++;
    if(_map[x][y-1])tot++;
...
void game_start(){
    ...
    while(!(posx==size&&posy==size)){
        char c=getch();
        if(c=='a')if(!_map[posx][posy-1]){
            _map[posx][posy]=0;
            posy-=1;
            _map[posx][posy]=-1;
        }
       if(c=='d')if(!_map[posx][posy+1]){
            _map[posx][posy]=0;
            posy+=1;
            _map[posx][posy]=-1;
        }
        if(c=='w')if(!_map[posx-1][posy]){
            _map[posx][posy]=0;
            posx-=1;
            _map[posx][posy]=-1;
        }
        if(c=='s')if(!_map[posx+1][posy]){
            _map[posx][posy]=0;
            posx+=1;
            _map[posx][posy]=-1;
        }
...
int main(){
    ...
    for(int x=1;x<=50;++x)
        for(int y=1;y<=50;++y)belong[x][y]=_hash(x,y);
    create_map();
    printf("\n\n\n\n");
    game_start();
    return 0;
