...
char letter[1000];
inline bool isWhite(const char &c) { return c == ' ' || c == '\n' || c == '\r' || c == EOF; }
inline void getLetter() {
    ...
    int cnt = 0;
    letter[cnt++] = c;
    while (!isWhite(c = getchar()) && cnt < length - 1)
        letter[cnt++] = c;
    letter[cnt] = '\0';
}
...
int main() {
    ...
    while (true) {
        ...
        putLetter();
        if (!dr)
            putchar(' '); // put a space to cover the character
        if (dr)
            ++now;
        else
            --now;
        if (now >= cmdlength - (int)strlen(letter) - 1 || now <= 0)
            dr = !dr; // change direction
        if (kbhit() && getch())
            break; // break if press a key
