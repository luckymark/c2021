  ...
  while(1)
    if(kbhit()){
      ...
      if (loster.y==endx&&loster.x==L-2){
        system("cls");
        cout<<"Congraduation!"<<endl<<"YOU WIN!"<<endl;
        break;
      }
    }
  system("pause");
...
bool judge(items &item,int **Maze){
  if(Maze[item.y][item.x]==WALL)return false;
  else return true;
}
void move(int input,items &item,int **Maze){
  int tempx=item.x;
  ...
  switch(input) {
    ...
    case 72: item.y--;break;
    case 75: item.x--;break;
    case 77: item.x++;break;
    case 80: item.y++;break;
    default: break;
  }
  if(judge(item,Maze)){
